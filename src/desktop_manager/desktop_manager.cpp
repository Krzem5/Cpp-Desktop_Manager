...
extern "C" BOOL __declspec(dllexport) move_to_desktop(HWND hwnd,UINT n){
	CoInitialize(NULL);
...
extern "C" BOOL __declspec(dllexport) switch_to_desktop(UINT n){
	CoInitialize(NULL);
	IServiceProvider* sp=nullptr;
	CoCreateInstance(CLSID_ImmersiveShell,NULL,CLSCTX_LOCAL_SERVER,__uuidof(IServiceProvider),(PVOID*)&sp);
	IVirtualDesktopManager* dm=nullptr;
...
BOOL APIENTRY DllMain(HMODULE m,DWORD r,LPVOID _r){
	return TRUE;
}
